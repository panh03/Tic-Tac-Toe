...
public class GameTicTacToe extends JFrame implements ActionListener {
	...
	public Container init() {
		...
		newGame_bt.setFont(new Font("Swiss 721 Black Condensed", 1, 18));
		newGame_bt.setActionCommand("-1");
		
		point1_bt = new JButton(String.valueOf(point1));
		point1_bt.setFont(new Font(" Nokia", 1, 25));
		
		point2_bt = new JButton(String.valueOf(point2));
		point2_bt.setFont(new Font(" Nokia", 1, 25));
	...
	public void addPoint(int k) {	
		if (!win) {
			...
			if (count == 0)
				luot.setText("X's turn");
			else
				luot.setText("O's turn");
			if (checkWin() != -1) {
				String mess = "";
				int cw = checkWin();
				if (cw == 2) {
